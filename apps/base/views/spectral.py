...
from ..forms.spectral import SpectralForm
import requests


class SpectralView(FormView):
    template_name = "spectral/spectral.html"
    form_class = SpectralForm
    success_url = reverse_lazy('base:spectral')
    extra_context = dict()
    paginate_by = 20

    def form_valid(self, form, **kwargs):

        url2 = "http://127.0.0.1:8000/spectral/"
        url= "https://backend-pythong1.herokuapp.com/spectral/"         
    ...
    def get_context_data(self, **kwargs):
        context = super(SpectralView, self).get_context_data(**kwargs)
